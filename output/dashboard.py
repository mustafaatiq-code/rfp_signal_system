...
with st.sidebar:
    ...
    # The Refresh button runs the live pipeline locally (needs network, a
    # browser for JS sources, and a writable DB). It is hidden on hosted
    # read-only deployments, where data is refreshed by publishing a new
    # snapshot instead (see DEPLOY.md).
    if not _is_deployed():
        st.divider()
